...
train_colors = [COLOR_ENERGY, COLOR_FORCE, COLOR_VIRIAL, COLOR_STRESS, COLORS["purple"], COLORS["gray"]]

test_colors = [COLORS["deep_blue"], "#D38A33", "#3F8F55", "#A94F4F", "#6F5A9A", COLORS["dark_gray"]]
...
def get_diag_color(data):
    ...
    if data == 'energy':
        ...
        return COLOR_ENERGY, '#ff9999'

    elif data == 'force':

        return COLOR_FORCE, '#ffcc80'

    elif data == 'virial':

        return COLOR_VIRIAL, '#81c784'

    elif data == 'stress':

        return COLOR_STRESS, '#ef9a9a'
...
def set_tick_params():
    ...
    tick_params(axis='x', which='both', direction='in', top=False, bottom=True,

                labelsize=24, length=5, width=1.0)

    tick_params(axis='y', which='both', direction='in', left=True, right=False,
...
def plot_value(values, color):
    ...
    if three_six_component == 0 or data == 'energy':
        ...
        plot(values[:, 1], values[:, 0], '.', color=color, markersize=3, alpha=0.70)

    else:

        for i in range(columns):

            plot(values[:, i+columns], values[:, i], '.', color=color[i % len(color)], markersize=3, alpha=0.70)
...
def loss_legend(labels, ncol, loc):

    """Compact legend used only for the loss plot."""
...
def plot_loss():
    ...
    ax.set_prop_cycle(color=[COLOR_ENERGY, COLOR_FORCE, COLOR_VIRIAL, COLOR_STRESS, COLORS['purple'], COLORS['gray'], COLORS['deep_blue'], '#D38A33'])
    ...
    if loss.shape[1] < 7:
        ...
        loglog(loss[:, 2:5], linewidth=1.35)

        if os.path.exists('test.xyz'):

            loglog(loss[:, 5], linewidth=1.35)

            loss_legend(label_Lnep + [f'{model_type}-train', f'{model_type}-test'], ncol=4, loc='upper right')

        else:

            loss_legend(label_Lnep + [f'{model_type}'], ncol=3, loc='upper right')

    else: 

        if lambda_v == '0' or (train_novirial_indices is not None and len(train_novirial_indices) == train_length):

            loglog(loss_L, linewidth=1.35)

            loglog(loss_train, linewidth=1.35)

            if os.path.exists('test.xyz'):

                loglog(loss_test, linewidth=1.35)
                ...
                loss_legend(loss_label, ncol=3, loc='lower left')

            else:

                loss_label = label_Lgnep + label_ef if os.path.exists('gnep.in') else label_Lnep + label_ef

                loss_legend(loss_label, ncol=4, loc='upper right')

        else:

            loglog(loss_L, linewidth=1.35)

            loglog(loss_train_v, linewidth=1.35)

            if os.path.exists('test.xyz'):

                loglog(loss_test_v, linewidth=1.35)
    ...
    if os.path.exists('gnep.in'):
        ...
        xlabel('Generation/100', fontsize=24)
...
def plot_learning_rate():
    ...
    plot(range(1, len(loss) + 1), loss[:, 8], linewidth=1.35)
...
def plot_diagonal(data):
    ...
    plot(linspace(range_min, range_max), linspace(range_min, range_max), '--', color=COLORS['dark_gray'], lw=1.1, zorder=0)

    set_tick_params()

    xlabel(f"DFT {data} ({label_unit})", fontsize=24)

    ylabel(f"NEP {data} ({label_unit})", fontsize=24)
    ...
    if data_test is not None:
        ...
        legend([f'train RMSE={rmse_train:.2f} {unit} R²={r2_train:.3f}',
               ...
               frameon=False, fontsize=12.5, loc='upper left', handletextpad=0.01,
...
def plot_charge():
    ...
    sns.set_palette([COLOR_ENERGY, COLOR_FORCE, COLOR_VIRIAL, COLOR_STRESS, COLORS['purple'], COLORS['gray']])
    ...
    if os.path.exists('charge_test.out'):
        ...
        legend(ncol=2, frameon=False, fontsize=24)
        ...
        legend(frameon=False, fontsize=24)

    xlabel('Charge', fontsize=24)
...
def plot_descriptor():
    ...
    try:
        ...
        if len(descriptor) == len(globals()['energy_train']):
            ...
            sc = scatter(proj[:,0], proj[:,1], c=globals()['energy_train'][:,1], cmap=ENERGY_CMAP, edgecolors='none', s=18, alpha=0.85)

            cbar = colorbar(sc, cax=gca().inset_axes([0.73,0.95,0.23,0.03]), orientation='horizontal')

            cbar.set_label('E/atom (eV)', fontsize=24)
            ...
            for i, elem in enumerate(ed):
                ...
                scatter([p[0] for p in ed[elem]], [p[1] for p in ed[elem]], s=18, alpha=0.80, label=elem, edgecolors='none')
...
def plot_element_force():
    ...
    for elem, fe in fet.items():
        ...
        plot(tef[:,1], tef[:,0], '.', color=COLOR_FORCE, label=f'{elem}-train', ms=3, alpha=0.70)
        ...
        if os.path.exists('force_test.out'):
            ...
            plot(ttef[:,1], ttef[:,0], '.', color=COLOR_ENERGY, label=f'{elem}-test', ms=3, alpha=0.70)

        rmin, rmax = get_range('force', tef)

        plot(linspace(rmin,rmax), linspace(rmin,rmax), '--', color=COLORS['dark_gray'], lw=1.1)

        set_tick_params()

        xlabel('DFT force (eV/Å)', fontsize=24)

        ylabel('NEP force (eV/Å)', fontsize=24)

        legend(frameon=False, fontsize=24)

        title(f'Force of {elem}', fontsize=24)

        tight_layout()

        save_png_pdf(f'{elem}-force')
...
def save_single_diagonal(data):

    """Generate each parity plot as a separate figure."""

    figure(figsize=(5.5, 5))

    plot_diagonal(data)

    save_png_pdf(f'nep-{data}-diagonal')
...
def plot_base_picture():
    ...
    if model_type:
        ...
        save_single_diagonal(model_type)

    else:

                                   

        if os.path.exists('energy_train.out'):

            save_single_diagonal('energy')

        if os.path.exists('force_train.out'):

            save_single_diagonal('force')

        if lambda_v != '0' and os.path.exists('virial_train.out') and len(train_novirial_indices) != train_length:

            save_single_diagonal('virial')

        if os.path.exists('stress_train.out'):

            save_single_diagonal('stress')
